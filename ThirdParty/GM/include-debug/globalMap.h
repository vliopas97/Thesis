...
namespace GM
{
	...
	public:
		...
		*/
		...
		@param ptr : The array's pointer on the GPU
		 ...
		 */
		static __host__ cudaError_t editGlobalMap(GlobalMapDevice** ptr, sl::Pose& pose, sl::Mat& mat, const sl::float4& planeEquation);

		/**
		\brief Updates the GlobalMap based on the numerical data from the newly grabbed camera frame
		but does not depend on any planar data
		@param ptr : The array's pointer on the GPU
		@param pose : The pose of the camera in current frame
		@param mat : The matrix holding the 3D point cloud for the current camera frame
		\Note For the mat input it is assumed that the floor plane lies at Z=0 in the World Coordinate System 
		 */
		static __host__ cudaError_t editGlobalMap(GlobalMapDevice** ptr, sl::Pose& pose, sl::Mat& mat);
		...
		*/
		...
		/**
		\brief Takes a 3-d point cloud given in reference to the camera position and converts it to woorld coordinates
		...
		*/
		static __host__ cudaError_t preprocess(sl::Mat& pointcloud, sl::Transform& transform);
